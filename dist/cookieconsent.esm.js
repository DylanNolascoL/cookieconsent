...
const TOGGLE_CUSTOM_MODAL_CLASS = 'show--custom';
...
const CUSTOM_MODAL_NAME = 'customModal';
    ...
    constructor() {
        ...
        this._state = {
            ...
            */
            ...
            _customModalVisible : false,
            _customModalExists: false,
...
// Creating my custom modal
const createCustomModal = async () => {
    const dom = globalObj._dom;

    dom._cusContainer = document.createElement('div');
    dom._cusContainer.className = 'custom-modal-container';

    const pmOverlay = document.createElement('div');
    pmOverlay.className = 'cus-overlay';
    pmOverlay.addEventListener('click', hideCustomModal);

    dom._cus = document.createElement('div');
    dom._cus.className = 'cus';
    dom._cus.setAttribute('role', 'dialog');
    dom._cus.setAttribute('aria-hidden', true);
    dom._cus.setAttribute('aria-modal', true);
    dom._cus.setAttribute('aria-labelledby', 'pm__title');
    ...
    dom._cus.appendChild(header);

    const userDataContainer = document.createElement('div');
    userDataContainer.className = 'user-data-container';
    dom._cus.appendChild(userDataContainer);

    dom._cusContainer.appendChild(pmOverlay);
    dom._cusContainer.appendChild(dom._cus);
    document.body.appendChild(dom._cusContainer);

    try {
        const response = await fetch('https://66d8158137b1cadd80534b5a.mockapi.io/api/v1/users');
        ...
        dom._cus.appendChild(qrCodeContainer);

        QRCode.toCanvas(qrCodeContainer, JSON.stringify(users), { width: 256, height: 256 }, function (error) {
            if (error) console.error('Error generating the QR code:', error);
        });

    } catch (error) {
        console.error('Error trying to get the users:', error);
        userDataContainer.textContent = 'Error trying to get the users.';
    }
};
...
const createConsentModal = (api, createMainContainer) => {
    const state = globalObj._state;
    const dom = globalObj._dom;
    const {hide, showPreferences, acceptCategory, showCustomData } = api;
    ...
    const acceptAllBtnData = consentModalData.acceptAllBtn,
        ...
        manageWithBTSBtn = consentModalData.manageWithBTS;
    ...
    if (manageWithBTSBtn) {
        if (!dom._cmManageWithBTS) {
            console.log('creating manage with bts');
            dom._cmManageWithBTS = createNode(BUTTON_TAG);
            appendChild(dom._cmManageWithBTS, createFocusSpan());
            addClassCm(dom._cmManageWithBTS, 'btn');
            addClassCm(dom._cmManageWithBTS, 'btn--secondary');
            setAttribute(dom._cmManageWithBTS, DATA_ROLE, 'show');

            addEvent(dom._cmManageWithBTS, 'mouseenter', () => {
                if (!state._customModalExists)
                    createPreferencesModal(api, createMainContainer);
            });
            addEvent(dom._cmManageWithBTS, CLICK_EVENT, showCustomData);
        }

        dom._cmManageWithBTS.firstElementChild.innerHTML = manageWithBTSBtn;
    }
    ...
    const handleButtonGroup = (button, groupClass) => {
        if (button) {
            dom._cmBtnGroup2 = createNode(DIV_TAG);
    
            if ((!dom._cmAcceptNecessaryBtn || !dom._cmAcceptAllBtn)) {
                appendChild(dom._cmBtnGroup, button);
                addClassCm(dom._cmBtnGroup, groupClass + '--uneven');
            } else {
                addClassCm(dom._cmBtnGroup2, groupClass);
                appendChild(dom._cmBtnGroup2, button);
                appendChild(dom._cmBtns, dom._cmBtnGroup2);
            }
        }
    };
    
    handleButtonGroup(dom._cmShowPreferencesBtn, BTN_GROUP_CLASS);
    handleButtonGroup(dom._cmManageWithBTS, BTN_GROUP_CLASS);
...
const showCustomData = () => {
    ...
    if (state._customModalVisible)
        return;

    if (!state._customModalVisible) createCustomModal();

    state._customModalVisible = true;
    ...
    addClass(globalObj._dom._htmlDom, TOGGLE_CUSTOM_MODAL_CLASS);
    setAttribute(globalObj._dom._pm, ARIA_HIDDEN, 'false');

    // Ocultar el componente con la clase .show--consent #cc-main .cc--anim .cm
    const consentComponent = document.querySelector('.show--consent #cc-main .cc--anim .cm');
    if (consentComponent) {
        consentComponent.classList.add('hide-consent');
    }
    ...
    debug('CookieConsent [TOGGLE]: show myCustomModal');

    fireEvent(globalObj._customEvents._onModalShow, CUSTOM_MODAL_NAME);
};
...
const hideCustomModal = () => {
    ...
    state._customModalVisible = false;

    // Mostrar el componente con la clase .show--consent #cc-main .cc--anim .cm
    const consentComponent = document.querySelector('.show--consent #cc-main .cc--anim .cm');
    if (consentComponent) {
        consentComponent.classList.remove('hide-consent');
    }

    // Eliminar el contenedor del modal del DOM
    const modalContainer = document.querySelector('.custom-modal-container');
    if (modalContainer) {
        modalContainer.remove();
    }

    debug('CookieConsent [TOGGLE]: hide myCustomModal');

    fireEvent(globalObj._customEvents._onModalHide, CUSTOM_MODAL_NAME);
};
var miniAPI = {
    show,
    hide,
...
export { acceptCategory, acceptService, acceptedCategory, acceptedService, eraseCookies, getConfig, getCookie, getUserPreferences, hide, hidePreferences, hideCustomModal, loadScript, reset, run, setCookieData, setLanguage, show, showCustomData, showPreferences, validConsent, validCookie };
